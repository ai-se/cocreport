...
\noindent
{\bf Comment to editors}

\noindent
Thank you for your details comments. 

\noindent
Based on that feedback,
we think we now have 3 papers:
\be 
\item This paper, describing a negative result;
\item Another paper describing our more positive result
on other non-cocomo data sets. Following the suggestions
of both reviewers, that material was removed
from this paper. Also, following the suggestion of Reviewer2,
we also applied the Wingham baseline method from TOSEM and found 
our methods were much much better. So we think that means
we have a ``better baseline'' paper to submit somewhere else.
\item
Yet another paper on the effects of different evaluation
criteria in effort estimation. At the excellent suggestion
of Reviewer2, we reran all our experiments using not MRE but 
using the ratio of the squared difference between the estimate and actual divided by the squared difference between the actual and the average effort. In terms of the conclusions offered by this paper,
it turns out that these two eval measures
produce the same results. We would prefer NOT to show the
equivalence of those results in this paper (since they add no
information to the argument of this paper) but instead have
added those charts in the private reply sections to our reviewers.
\ee
There is one matter for which your editorial opinion is required:
\bi
\item
We think it is valid to include in this paper four results: two in
the public domain and two from our own private sources.
\item
Reviewer2 does not, describing those results as ``hersay''
\ei
We fully agree with Reviewer2 that it is less-than-desirable
to publish results that are not repoducible. But:
\bi
\item
Many, many other empirical SE papers report results on
proprietary data sets (e.g. most the results coming out of Microsoft from Zimmermann and Bird).
\item
Our general results hold across all our data sets so our results are reproducible on the 2 data sets that we do publishs.
\item 
We have tried, most earnestly, for decades to collect
and publish  data sets (see the 500+ data sets at openscience.us-- a web site we created and maintain for the SE community).
...
\ei
So that is our case. If the editors agree with reviewer2 on this matter, they should:
\bi
\item Order another paper revision where we dump two of our four data sets. We do not prefer this option since our case is stronger with four data sets than two.
\item Or they could reject the paper.
\ei
\newpage
\date{}
%
...
\institute{T. Menzies, G. Mathew \at
                CS, North Carolina State Univ., USA
                \email{tim.menzies@gmail.com}, \email{george.meg91@gmail.com}
            \and
            Y. Yang \at
                SSE, Stevens Inst., USA
                \email{yangye@gmail.com}
            \and
            B. Boehm \at
                CS, Univ. of Southern California, USA
                \email{barryboehm@gmail.com}
            \and
            J. Hihn \at
                JPL, CalTech, USA
                \email{jairus.hihn@jpl.nasa.gov}
}
...
\begin{abstract}
%\boldmath
{\em Context:} More than half the literature on software effort estimation (SEE) focuses on 
comparisons of new estimation methods. Surprisingly, there are no studies comparing 
state of the art latest methods with decades-old approaches.

{\em Objective:} To check if new   SEE methods generated
better estimates than older methods.

{\em Method:} {\em Firstly}, collect effort estimation methods ranging from ``classical'' COCOMO
(parametric estimation over a pre-determined set of attributes) to ``modern'' (reasoning via analogy
using spectral-based clustering plus instance and feature selection, a recent ``baseline method'' proposed
in ACM Transactions on Software Engineering).  {\em Secondly}, catalog the list of objections that lead to the development of
post-COCOMO estimation methods. {\em Thirdly}, characterize each of those objections as a
comparison between newer and older estimation methods. {\em Fourthly},  using four COCOMO-style
data sets (from 1991, 2000, 2005, 2010), run those comparisons experiments.
{\em Fifthly}, compare the performance of the different estimators using a Scott-Knott procedure  using (i) the A12 effect size to
rule out ``small'' differences and (ii) a 99\% confident bootstrap procedure to
check for statistically different groupings of treatments). 

{\em Results:} The {\bf major negative result} of this paper is that for the COCOMO data sets, nothing we studied did any 
better than Boehm's original  procedure.  

{\em Conclusions:} When COCOMO-style attributes are available, we strongly recommend using
that data since the experiments
of this paper show that, at least for effort estimation,
{\em how data is collected} is more important than {\em what learner is applied to that data}.
...
there are very few failure reports  in the field of software analytics. 
Some authors  include a ``Erratta''
section in the papers that patch older conclusions.
However, such reports are infrequent (evidence: we know of only two such report
in the last five years of  conferences on software analytics~\cite{menzies13err,Murphy-Hill2012}).
Given the complexity of software analytics, this absence of such failure reports is highly suspicious. 
...
superior method is actually overly complex. Hence we take care to compare methods
against simpler alternative. 

Accordingly, this paper checks
...
To guide our exploration,
 this paper asks five research questions.  These
...
if there are large errors in the ``thousand lines of code''(KLOC) estimate.
...
Hence, as seen in RQ5,  we can make some degree of
error in our KLOC estimates without damaging the overall estimation process (the caveat here is that if the KLOC
estimates grow too erroneous-- more than 200\%--
then some degradation is seen).
...
and (2)~estimates are always better done using expert-based guess-timation, a.k.a. {\em Delphi-based} methods\footnote{The mythological oracle of Delphi spoke for the god Apollo to answer questions  about colonization, religion, and power.}.
...
there is no clear case that Delphi-methods are better.
\item 
...
Note that it is far easy to apply this double-check strategy using Delphi+model-based methods 
...
\end{equation}
\begin{BLUE}
From  \eq{ration}, we can make
two observations. Firstly,   changes to the scale
factors and effort multipliers have a large
effect on effort (up to a factor of 856).
Secondly, 
the difference
between the min and max estimate differs as a very low-order
polynomial ($\mathit{KLOC}^{0.25}$). That is, we might expect
changes other that KLOC to
have a relatively larger impact on   COCOMO's
estimates. 

To confirm this second possibility we ran two small experiments
with the COCOMO equation. In both experiments,
project size was varied from 0 to 1,000,000 lines of code.
For each size, two estimates were generated using 
by adjusting the effort/scale factors to their (1)~{\em least}
and (2)~{\em most} values. As  shown in \fig{linear},
the exponent on the KLOC term is very close to one while,
in the worst case (on the orange curve) other factors
can change the estimate by three orders of magnitude (as witnessed
by the leading 1699.07 value). 
\end{BLUE}
...
\begin{figure}[!h]
\includegraphics[width=5in]{linear.png}
\caption{\textcolor{blue}{COCOMO estimates increase
as very low-order power series.
Assuming {\em most} / {\em least} values for
effort and scale factors, the exponent
on the KLOC term is 0.9723 and 1.23, respectively.}}\label{fig:linear}
\end{figure}
...
In this section, we discuss the methods used to explore the reserach questions defined
in the introduction.
%% \begin{figure*}
%% {\scriptsize
%% \begin{verbatim}
...
not unduly affected by noise where the KLOC values
...
on lines of code be immune to errors in measures of those lines of code? 
Secondly, 
...
it can be shown that this result can be explained by the underlying theory of COCOMO.
 ...
 Recall that \eq{ration} showed
how    effort   varies for a given KLOC. While KLOC was a factor in that equation,
all the other COCOMO variables have a large influence on effort estimation. This means that even if there are errors in the KLOC measure, the other COCOMO variables can ``step in'' to comment on effort estimation.
...
have better medians than once noise is injected). So it is not true to
say that KLOC noise has no effect on effort estimation.
That said, it should also be noted
that statistically, the size of that effect is usually very small  or not
significant (as defined by the statistical tests of \tion{stats}).
...
on variance reduction, after which, they can better explore reducing median error values.
