...
using spectral-based clustering plus instance and feature selection, and \textcolor{blue}{a recent ``baseline method'' proposed
...
{\em Conclusions:} When COCOMO-style attributes are available, we strongly recommend (i)~using
that data and (ii)~use COCOMO to generate predictions.
We say this since the experiments
...
%error in our KLOC estimates without damaging the overall estimation process (the cav%eat here is that if the KLOC
...
\mathit{e2}=\frac{ (\mathit{actual} - \mathit{predicted})^2}{(\mathit{actual - average(actual)})^2}
\end{equation}
...
not change our conclusion. Hence, we can report that our
...
%\item {\bf RQ5}: Parametric estimation methods like COCOMO that assume effort is exp%onential on lines of code are {\em not} unduly
...
\item Further, at the suggestion of Reviewer\#2, we re-ran all our results
  ...
  you and the reviewers to inspect (at \url{https://goo.gl/9KvvOa})-- but it adds no information to this
...
to a wide range of projects.
     ...
     What we should have written was $\frac{3a + 2b + 1c}{6}$. That has been fixed now at  \WHERE{Reviewer1c}.
...
\noindentFixed (and thanks for pointing out those errors).
...
\begin{quote}{\em  Generally, the paper has a large number of different elements  that makes it difficult to follow. If you are interested in the value of COCOMO-II why not stick to that.\end{quote}
}
...
Also, at your suggestion, we ran more experiments (with different error messages,
...
\WHERE{Reviewer2a} and \WHERE{Reviewer2b} and  \WHERE{Reviewer2d}.
...
please  see https://goo.gl/5Vnocl.  
...
NASA10 is now publicly available (and included in this paper, \WHERE{Reviewer2c}).
...
As part of a revised threats to validity section on \WHERE{Reviewer2e}, we 
...
your  inspection at \url{https://goo.gl/9KvvOa}). 
...
\begin{quote}{\em Whingham et al.'s default model seems more
...
\WHERE{Reviewer2a} and \WHERE{Reviewer2b} and \WHERE{Reviewer2d}. And please note, as we stated
before, how badly
Whingham performs in \WHERE{Reviewer2d}-- thanks to your comments on this matter, we know have
...
We have added a note to this effect at \WHERE{Reviewer2e}.
...
We double checked the raw data (KLOCs and effort)
...
\begin{quote}{\em 4.4 It would be useful to plot the actual size against the recalculated size. If the revised size is still correlated with the actual size you may only have demonstrated that the exponential term in the COCOMO model is close to 1.}
