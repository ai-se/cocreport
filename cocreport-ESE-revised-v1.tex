...
using spectral-based clustering plus instance and feature selection, and \textcolor{black}{a recent ``baseline method'' proposed
...
data sets (from 1991, 2000, 2005, 2010) and run those comparisons experiments.
...
{\em Results:} The {\bf major negative results} of this paper are that for the COCOMO data sets, nothing we studied did any 
...
\end{equation}
This measure is derived from {\em Standardized Accuracy}($SA$) defined by Shepperd \& MacDonnell~\cite{shepperd12a} ($SE = 1 - SA$). In Equation ~\ref{eq:se}, $actual$ represents the true value, $predicted$ represents the estimated value by the predictor and $sampled$ is a value drawn randomly from a list of random samples(with replacement) from the training data.  Shepperd and MacDonnell also propose
...
\input{results/coc_vs_loc}


\input{results/coc_vs_standard}
...
and explore non-parametric methods (PEEKING2 and TEAK) as well
...
% the SE error measure defined in \eq{se}.
...
% the performance measure from \eq{se} to  \eq{e2} does
% not change our conclusion. More specifically,
% despite changing from \eq{se} to  \eq{e2}:
% \begin{quote}
...
Having endorsed the use of parametric methods such as COCOMO, it is appropriate
...
multi-core chips, field-programmable-gate-array
