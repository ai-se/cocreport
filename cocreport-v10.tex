...
Note that all the non-proprietary data used in this study (COC81 and NASA93) is available at
...
\end{center}
\caption{Projects in this study (COC81 is a subset of COC'2000).}\label{fig:dataused}
\end{figure}
...
The data used to build this model is largely different to the four test sets used in the experiments
of this paper: 
\bi
...
are proprietary and 
cannot be released to the research community). These projects have no overlap
to each other other,
as well as with COC81 and NASA93 (see \fig{types}).  
Further, except COC81, all other data comes from different projects than the 161 project used train the current  COCOMO.
\ei
...
The concern here is that  this is not enough data to constrain the learning of a 23 parameter
...
estimates are always better done using expert-based guess-timation, a.k.a. {\em delphi-based} methods.
While it is true that delphi-based estimation is a common practice~\cite{boehm00a}, this is not to say that it (a) leads to good results; or (b) is easy to apply. In fact, drawbacks with  delphi methods are one of the main motivators for investigating model-based 
approaches:
\bi 
...
\item
Jorgensen \& Gruschke document how
  commercial estimation ``gurus'' rarely use lessons
  from past projects to improve their future estimates~\cite{jorgensen09}. 
They note that when engineers
  fail to revise their beliefs, this leads to poor
  Delphi-based estimates  (see examples in~\cite{jorgensen09}).
  \ei 
Once of us (Hihn) is adamant that best estimations come from combining the predictions
from multiple oracles.  The issue with using multiple oracles is that some of them
can be very slow to reach their conclusions. Valerdi has carefully documented how
long it takes a panel of experts to reach consensus on the estimates for XXX

Another matter relating to ease of use is when can humans perform effort estimates.
Once a
...
estimates, the Delphi approach has some problems. This is an important issue since, for larger
