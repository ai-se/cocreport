...
\lstset{
    ...
    emph={COCONUT,GUESSES,ASSESS,COCOMO2,PEEKER,SAMPLE,WHERE,RIG}, emphstyle=\bfseries\color{Blue},
...
According, we conclude that (a)~it is still valid and recommended practice to try parametric estimation
...
%% PEEKER algorithm of Papakroni~\cite{papa13}.
%% PEEKER
...
%% the closer centroid has more weight, see \fig{PEEKER}).
%% That is,  PEEKER is any example of a {\em prototype learner}~\cite{chang74}
...
%% Papakroni~\cite{papa13} found that PEEKER's predictions where (1)~ competitive
...
%% \end{lstlisting}
%% \caption{PEEKER: a non-parametric response surface method that 
%% generates estimates from a  set of piecewise linear approximations.
%% PEEKER builds its clusters using the WHERE spectral clusterer~\cite{me11m}
%% that
...
\bi
\item
PEEKER's feature weighting scheme changes the $_w_i$ score used in the distance 
...
is computed using \eq{dict} plus feature weighting). If these prototypes are found at distance
...
see Shepperd\&MacDonell~\cite{shepperd12a}; Kampenes~\cite{kampenes07}. These researchers
...
\caption{COCOMO vs newer methods. Displayed as per \fig{loc}.}\label{fig:fss}
\end{figure}
...
\newcommand{\rone}{\crule[red]{1mm}{1.95mm}}

\newcommand{\rtwo}{\crule[red]{2mm}{1.95mm}}

\newcommand{\rthree}{\crule[red]{3mm}{1.95mm}}

\newcommand{\rfour}{\crule[red]{5mm}{1.95mm}}

\newcommand{\rfive}{\crule[red]{5mm}{1.95mm}}
...
\begin{tabular}{r@{~}|r@{~}|r@{~}|l@{~}|r@{~}l@{~}|r@{~}l@{~}|r@{~}l@{~}|r@{~}l@{~}|r@{~}l}
  \multicolumn{4}{c|}{~}&\multicolumn{9}{c}{median( {\em abs(predicted - actual) / actual} ) } \\
  Data set   &   cols   &   rows   &   notes   &   \multicolumn{2}{c}{PEEKER}         &   \multicolumn{2}{c}{Knear(3)}         &   \multicolumn{2}{c}{Knear(1}         &   \multicolumn{2}{c}{CART~}    &   \multicolumn{2}{c}{Lin.Regr.}\\\hline
\multicolumn{1}{c}{~}\\
Albrecht  &   7         &   24   &   Projects from IBM   &   28   &       &   38   &   {\rone}   &   38   &   {\rone}   &   40   &   {\rone}   &   57   &   {\rfour}\\
China      &   18       &   488   &   Projects from Chinese software companies   &   38   &       &   34   &       &   35   &       &   34   &       &   52   &   {\rfour}\\
Cosmic     &   y   &   y   &   Projects described in functiion points   &   98   &   {\rone}   &   75   &       &   85   &       &   85   &       &   135   &   {\rfour}\\
ISBSG10    &   y   &   y   &   From the ISBSG benchmark suite   &   56   &       &   62   &   {\rone}   &   66   &   {\rone}   &   77   &   {\rfour}   &   78   &   {\rfour}\\
Kemerer    &   7   &   15   &   Large business applications   &   24   &      &   42   &   {\rtwo}   &   55   &       &   55   &   {\rtwo}   &   74   &   {\rfour}\\
Kitchenham &    y   &   y   &  y    &   34   &   {\rone}  &   43   &   {\rfour}   &   43   &   {\rfour}   &   34   &   {\rone}   &   28   &    \\
Maxwell    &   27  &   62  & Projects from commercial banks in Finland   &   57   &   {\rtwo}   &   56   &   {\rone}   &   53   &   {\rtwo}   &   47   &       &   63   &   {\rfour}\\
Miyazaki   &   8   &   48 &Japanese software projects developed in COBOL   &   29   &       &   41   &   {\rtwo}   &   47   &   {\rone}   &   41   &   {\rone}   &   54   &   {\rfour}\\
Telecom    &   3   &   18   &   Maintenance projects for telecom companies   &   23   &       &   26   &   {\rone}   &   31   &       &   31   &   {\rone}   &   44   &   {\rfour}\\
Usp05      &   y   &   y   &   y   &   75   &   {\rone}   &   50   &       &   25   &       &   45   &       &   140   &   {\rfour}\\
\end{tabular}
\end{center}
\caption{Median errors seen in leave-one-out studies on
non-COCOMO data sets. Red bars
show the errors for each data set (and the {\em shorter} the bar, the {\em better} the effort estimator).
These bars  show the error 
discretized into 20th percentiles ranges from min to max (discretization performed seperately
for each data set).
An absent bar denotes errors in the 0th to 20th percentile range for that data set.
The longest bar denote largest
errors in the 80th to 100th percentile range for that data set. Note that PEEKER
 has the smallest errors while 
linear regression most often has the largest (exception: Kitchenham).}
\end{figure*}
