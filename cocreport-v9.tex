...
``no''. It turns out that RQ1 and RQ5 are connected.
As discussed later in this paper, there is more to effort estimation than merely
consider KLOC estimates. Hence,  just as estimation using merely
KLOC is not recommended (see RQ1), we can make some degree of
...
\section{Overall Structure of These Experiments}\label{sect:overall}

When designing a data mining experiment, it is vitally
important {\em not} to  evaluate the learned model using the data used to learn that model.
...
Note that while all of these data sets could be called ``COCOMO'' models (since they all use the
same column names), their rows come from very different projects:

To be  specific, the canonical  model used as ``the'' COCOMO model in this study was
trained from  161 projects in the period 1982 to 1997 from commercial, aerospace,government, and non-profit organizations. Our other four data sets, used  for testing,  are:
\bi
...
Further, all this data comes from different projects than the 161 project used train ``the'' canonical   COCOMO.
\ei
...
model-- which means that that data could be fitted to almost anything.
...
Hence, it is important to compare standard methods
with {\em dimensionality reduction methods} that first ``trim'' the data and hence the number of the parameters in the model. This study explores two such ``trimming'' methods (the COCONUT and TEAK tools, described below). Surprisingly, in a result that strengthens the case for ``the'' canonical COCOMO  model, we show below that trimming
...
Before going on, we offer an  equation that will be required later
in this paper. For a fixed KLOC, \eq{cocII} tells us that
the minimum  
effort estimate is bounded by the  {\em sum} of the minimum scale factors
and the {\em product} of the minimum effort multipliers (and similarly for
the maximum effort estimate). From \fig{coc2},
that range is:
\begin{equation}\label{eq:range}
0.18*\mathit{KLOC}^{0.97}  \le \mathit{effort} \le 154*\mathit{KLOC}^{1.23}
\end{equation}
Dividing the minimum and maximum values shows
how   a COCOMO estimate can vary for a given KLOC:
...
Note that leave-one-out ensures that the our models
are tested on data never seen during training.
...
The performance of our estimators was
